...
public class Calculator extends VBox implements EventHandler<ActionEvent>{
	...
	public void handle(ActionEvent event) {
		...
		if(value.equals("C")) {//Si es C se borran los valores almacenados y se lleva "" a display
			displayText.setText("");
			number1 = "";
			number2 = "";
			operator = "";
		}else if(value.equals("=")){//Si es = se valida que todo esté almacenado para la correcta operación y realiza la respectiva operación
			if(!number1.equals("") && !number2.equals("") && !operator.equals("")) {
				if(operator.equals("+")) {
					...
					displayText.setText(String.valueOf(Float.parseFloat(number1)/Float.parseFloat(number2)));
				}
				number1 = "";//Se reinician los valores para una nueva operación
				number2 = "";
				operator = "";
			}
	
		}else {//En caso contrario se valida si es un operador
			if(value.equals("+") || value.equals("-") || value.equals("*") || value.equals("/")){
				operator = value;
				displayText.setText(value);
			}else {//Sino se guarda el number1 y el number2 dependiendo sea el caso (nota: se permiten numeros de mas de una cifra)
				if((number1.equals("") || operator.equals("")) && value.matches("[+-]?\\d*(\\.\\d+)?")) {
					number1 += value;
					displayText.setText(number1);
				}else if(number2.equals("") || value.matches("[+-]?\\d*(\\.\\d+)?")) {
					number2 += value;
